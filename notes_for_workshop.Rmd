...
```{r}
phenotype %>% select(airway_stenosis, diagnosis_as_gsd) %>% group_by(airway_stenosis, diagnosis_as_gsd) %>% tally
```
...
Let's subset our cases to only include patients who checked the box and who also have a GS diagnosis.


```{r}
airway.stenosis.cases <- phenotype %>% filter(diagnosis_as_gsd=="Yes") %>% select(participant_id) %>% inner_join(airway.stenosis.cases, by="participant_id")
airway.stenosis.cases
```
...
Looking at this table, note that there are some tasks that have the numbers we would expect (64/59) and others that have imbalances; these are likely stimuli from other protocols that some of our cohort happeend to also be eligible for, and for which they were cases or controls.
...
Do some variable centering and nominalization:
...
simple_glm_clf <- predict(simple_glm, airway.clf.matrix.split.test, type="class") %>% bind_cols(airway.clf.matrix.split.test)
